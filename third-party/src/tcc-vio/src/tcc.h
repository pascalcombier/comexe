...
# define CONFIG_SYSROOT "COMRAD-RUNTIME-V2"
#endif

/* ComRAD PATCH: set CONFIG_TCCDIR */
#ifndef CONFIG_TCCDIR
# if defined(_WIN32)
#  define CONFIG_TCCDIR CONFIG_SYSROOT "/usr/lib/tcc-x86-64-windows-runtime"
# else
#  define CONFIG_TCCDIR CONFIG_SYSROOT "/usr/lib/tcc-x86-64-linux-runtime"
# endif
...
# else
# define CONFIG_TCC_CRTPREFIX USE_TRIPLET(CONFIG_SYSROOT "/usr/" CONFIG_LDDIR "/x86_64-linux-musl")
#endif
#endif
...
#  define CONFIG_TCC_SYSINCLUDEPATHS "{B}/include"PATHSEP"{B}/include/winapi"PATHSEP CONFIG_SYSROOT CONFIG_USR_INCLUDE
# else
#  define CONFIG_TCC_SYSINCLUDEPATHS \
        "{B}/include" \
    ":" CONFIG_SYSROOT CONFIG_USR_INCLUDE "/x86_64-linux-musl" \
...
#  define CONFIG_TCC_LIBPATHS \
        "{B}" \
    ":" CONFIG_SYSROOT "/usr/" CONFIG_LDDIR "/x86_64-linux-musl" \
...
#else
ST_DATA pthread_mutex_t TccPreprocessorMutex;
#endif

/* Preprocessor state management functions */
ST_FUNC void tcc_preprocessor_state_enter(TCCState *s1);
ST_FUNC void tcc_preprocessor_state_leave(void);
...
ST_FUNC ssize_t full_read(TCCState *s1, int fd, void *buf, size_t count);
ST_FUNC void *load_data(TCCState *s1, int fd, unsigned long file_offset, unsigned long size);
ST_FUNC int tcc_object_type(TCCState *s1, int fd, ElfW(Ehdr) *h);
...
ST_FUNC char* macho_tbd_soname(TCCState *s1, int fd);
#endif
#endif
/* ------------ tccrun.c ----------------- */
#ifdef TCC_IS_NATIVE
