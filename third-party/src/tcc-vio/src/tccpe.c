...
static int get_dllexports(TCCState *s1, int fd, char **pp)
{
    int i, k, l, n, n0, ret;
    char *p;
    ...
    p = NULL;
    ret = 1;
    if (!read_mem(s1, fd, 0, &dh, sizeof dh))
        goto the_end;
    if (!read_mem(s1, fd, dh.e_lfanew, &sig, sizeof sig))
        goto the_end;
    if (sig != 0x00004550)
        goto the_end;
    pef_hdroffset = dh.e_lfanew + sizeof sig;
    if (!read_mem(s1, fd, pef_hdroffset, &ih, sizeof ih))
        goto the_end;
    opt_hdroffset = pef_hdroffset + sizeof ih;
    if (ih.Machine == 0x014C) {
        IMAGE_OPTIONAL_HEADER32 oh;
        sec_hdroffset = opt_hdroffset + sizeof oh;
        if (!read_mem(s1, fd, opt_hdroffset, &oh, sizeof oh))
            goto the_end;
    ...
    for (i = 0; i < ih.NumberOfSections; ++i) {
        if (!read_mem(s1, fd, sec_hdroffset + i * sizeof ish, &ish, sizeof ish))
            goto the_end;
        //printf("vaddr: %08x\n", ish.VirtualAddress);
...
found:
    ...
    if (!read_mem(s1, fd, addr - ref, &ied, sizeof ied))
        goto the_end;
    k = ied.NumberOfNames;
    if (k) {
        namep = tcc_malloc(l = k * sizeof *namep);
        if (!read_mem(s1, fd, ied.AddressOfNames - ref, namep, l))
            goto the_end;
        for (i = l = 0; i < k; ++i) {
            p1 = namep[i] - ref;
            if (p1 != p0)
...
static int pe_load_res(TCCState *s1, int fd)
{
    ...
    if (!read_mem(s1, fd, 0, &hdr, sizeof hdr))
        goto quit;
    ...
    if (!read_mem(s1, fd, offs, ptr, hdr.sectionhdr.SizeOfRawData))
        goto quit;
    ...
    for (i = 0; i < hdr.sectionhdr.NumberOfRelocations; ++i) {
        ...
        if (!read_mem(s1, fd, offs, &rel, sizeof rel))
            goto quit;
...
ST_FUNC int pe_load_file(struct TCCState *s1, int fd, const char *filename)
{
    ...
    if (0 == strcmp(tcc_fileextension(filename), ".def"))
        ...
        ret = 0;
    else if (read_mem(s1, fd, 0, buf, 4) && 0 == memcmp(buf, "MZ", 2))
