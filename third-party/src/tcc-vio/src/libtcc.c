...
static int tcc_compile(TCCState *s1, int filetype, const char *str, int fd, const char *filename)
{
    ...
    tcc_enter_state(s1);
    /* tccpp.c is not reentrant */
    tcc_preprocessor_state_enter(s1);
...
#undef gnu_ext
    ...
    /* we always want static and never dynamic */
    s->static_link = 1;
...
LIBTCCAPI void tcc_set_userdata (TCCState *state, void *user_data)
{
    state->user_data = user_data;
}

LIBTCCAPI void *tcc_get_userdata (TCCState *state)
{
    return state->user_data;
}

/* To avoid patching the whole tccpp.c and add TCCState everywhere, we
 * consider the preprocessing as an atomic operation run in a
 * critical section
 */
...
#ifdef _WIN32
        InitializeCriticalSection(&TccPreprocessorMutex);
#else
        pthread_mutex_init(&TccPreprocessorMutex, NULL);
#endif
        mutex_initialized = 1;
    }

    /* Enter critical section and set global preprocessor state */
#ifdef _WIN32
    EnterCriticalSection(&TccPreprocessorMutex);
#else
    pthread_mutex_lock(&TccPreprocessorMutex);
#endif
    PreprocessState = s1;
}
...
{
    /* Reset global preprocessor state and leave critical section */
    PreprocessState = NULL;
#ifdef _WIN32
    LeaveCriticalSection(&TccPreprocessorMutex);
#else
