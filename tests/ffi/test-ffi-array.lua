...
PairValueItem:set("First", 123)
PairValueItem:set("Second", 456)
PairValueArray:set(1, PairValueItem)

local PairValueRead = PairValueArray:get(1)
assert((type(PairValueRead) == "table"))
assert((PairValueRead:get("First") == 123))
assert((PairValueRead:get("Second") == 456))
...
assert((ZeroedValue:get("First") == 0))
assert((ZeroedValue:get("Second") == 0))

PointerArray:set(1, "abc")
local ReadString = PointerArray:get(1)
assert((type(ReadString) == "userdata"))
local ReadBack = ffi.readstring(ReadString)
assert((ReadBack == "abc"))
-- Reset to NULL for subsequent tests
PointerArray:set(1, nil)
local NullValue = PointerArray:get(1)
assert(NullValue == ffi.NULL)
